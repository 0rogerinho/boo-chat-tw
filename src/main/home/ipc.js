...
let configWin = null;
export const registerIPC = (win) => {
    ipcMain.handle('get-system', () => {
        console.log('process.platform', process.platform);
        return process.platform;
    });
    ipcMain.on('setFullScreen', (_event, showFullscreen) => {
    ...
    ipcMain.on('setIgnoreMouseEvents', (_event, value) => {
    ...
    ipcMain.handle('fetch-youtube', async (_event, url, payload) => {
        try {
            const options = {
